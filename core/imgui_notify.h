...
#define NOTIFY_DEFAULT_DISMISS			3.0		// Auto dismiss after X ms (default, applied only of no data provided in constructors)
...
public:
	...
	NOTIFY_INLINE auto get_elapsed_time() { return GetEngineTime() - this->creation_time; }
	...
	NOTIFY_INLINE auto get_fade_percent() -> const float
	{
		...
		{
			return (1.f - (((double)elapsed - (double)NOTIFY_FADE_IN_OUT_TIME - (double)this->dismiss_time) / (double)NOTIFY_FADE_IN_OUT_TIME)) * NOTIFY_OPACITY;
		}
...
public:
	...
	ImGuiToast(ImGuiToastType type, double dismiss_time = NOTIFY_DEFAULT_DISMISS)
	{
		...
		this->creation_time = GetEngineTime();
	...
	ImGuiToast(ImGuiToastType type, double dismiss_time, const char* format, ...) : ImGuiToast(type, dismiss_time) { NOTIFY_FORMAT(this->set_content, format); }
};
...
namespace ImGui
{
	...
	/// </summary>
	NOTIFY_INLINE void RenderNotifications(ImVec2 vp_size)
	{
		float height = 0.f;
